...
namespace InjectorMono
{
    class Program
    {
        ...
        {
            string Namespace = "Desire";
            string Class = "Pro";
            string Method = "Gamer";
            if (Process.GetProcessesByName("RustClient").Length != 0)
            {
                IntPtr remoteAssembly = IntPtr.Zero;
                Process process = Process.GetProcesses().FirstOrDefault(p => p.ProcessName.Equals("RustClient", StringComparison.OrdinalIgnoreCase));
                DLL dll = new DLL();
            ...
            Console.WriteLine("Open RustClient BRO...");
            Console.ReadKey();
        }
        #region DllImport
